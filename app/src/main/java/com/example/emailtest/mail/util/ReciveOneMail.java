...
public class ReciveOneMail {  
    ...
    public void saveAttachMent(Part part) throws Exception {  
        ...
        if (part.isMimeType("multipart/*")) {  
            Multipart mp = (Multipart) part.getContent();  
            int number = mp.getCount();
            for (int i = 0; i < number; i++) {  
                BodyPart mpart = mp.getBodyPart(i);//主体部分得到处理  
                String disposition = mpart.getDisposition();  
                if ((disposition != null)&& ((disposition.equals(Part.ATTACHMENT)) || (disposition.equals(Part.INLINE)))) {//ATTACHMENT附件，INLINE嵌入  
     ...
     * 【获取附加个数】  
     */  
    public int getCount(Part part) throws Exception {  
    	int number = 0;
        if (part.isMimeType("multipart/*")) {  
            Multipart mp = (Multipart) part.getContent();  
            number=mp.getCount();
