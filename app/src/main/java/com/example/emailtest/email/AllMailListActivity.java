...
public class AllMailListActivity extends Activity implements  OnClickListener {

	private ListView mailListView;
	private List<MailBrief> list = new ArrayList<MailBrief>();
	private MailAdapter mailListAdapter;
	private User user;
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		setContentView(R.layout.activity_allmail_list);
		...
		mailListView = (ListView) findViewById(R.id.mail_list);
		
		list = mailService.findMailAll();
		tv_title.setText("所有收件箱("+list.size()+")");
		
		mailListAdapter = new MailAdapter(this, list, user);
		mailListView.setAdapter(mailListAdapter);
		mailListView.setOnItemClickListener(new OnItemClickListener() {
			@Override
			public void onItemClick(AdapterView<?> parent, View view, int position, long id) {
				Intent intent = new Intent(AllMailListActivity.this, MailDetailActivity.class);
				intent.putExtra("user", userService.findByUserId(list.get(position-1).getUserId()));
				...
				SharedPreferencesUtils.putValue(AllMailListActivity.this, user.getUsername(),
	...
	public void onClick(View v) {
		...
		case R.id.image_back:{
			AllMailListActivity.this.finish();
		}
			break;
		case R.id.image_edit:{
			Intent intent = new Intent(AllMailListActivity.this,WriteActivity.class);
			startActivity(intent);
		}
			break;
		default:
	...
	public class MailAdapter extends BaseAdapter {
		...
		public MailAdapter(Context context, List<MailBrief> list, User user) {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			// 做的本地假已读未读
			boolean isBoolean = SharedPreferencesUtils.getValue(context,userService.findByUserId(mb.getUserId()).getUsername(), mb.getSubject()+mb.getSendTime(), false);
